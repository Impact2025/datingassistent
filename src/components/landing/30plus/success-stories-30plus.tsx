'use client';

import { motion } from 'framer-motion';
import { Star, Quote } from 'lucide-react';
import { Badge } from '@/components/ui/badge';

const colors = {
  deepPurple: 'var(--color-deep-purple, #722F37)',
  dustyRose: 'var(--color-dusty-rose, #E3867D)',
  softBlush: 'var(--color-soft-blush, #F5E6E8)',
  warmCoral: 'var(--color-warm-coral, #FF7B54)',
  charcoal: 'var(--color-charcoal, #2D3142)',
  mediumGray: 'var(--color-medium-gray, #6B6B6B)',
};

const stories = [
  {
    quote: "Na twee serieuze relaties die op niets uitliepen wist ik niet meer wat ik deed. De emotionele ready scan liet me zien dat ik steeds dezelfde keuzes maakte. Nu, 4 maanden later, date ik iemand die echt bij mij past.",
    name: 'Laura',
    age: 34,
    situation: 'Single na lange relatie',
    program: 'Transformatie',
    timeToSuccess: '4 maanden',
  },
  {
    quote: "Drukke baan, weinig tijd - ik dacht dat serieus daten gewoon niet haalbaar was. Het programma hielp me slim te daten: minder apps, betere matches. Binnen 6 weken mijn eerste écht goede date gehad.",
    name: 'Tim',
    age: 31,
    situation: 'Workaholic, nooit tijd',
    program: 'Kickstart',
    timeToSuccess: '6 weken',
  },
  {
    quote: "Ik was klaar met apps maar wist niet hoe ik anders moest zoeken. Leerde patronen herkennen en een profiel bouwen dat ik écht trots op ben. Op mijn 37e voel ik me zekerder dan ooit.",
    name: 'Mila',
    age: 37,
    situation: 'App-moe, wil echt contact',
    program: 'VIP Reis',
    timeToSuccess: '2 maanden',
  },
];

export function SuccessStories30Plus() {
  return (
    <section className="py-20 px-4" style={{ backgroundColor: colors.softBlush }}>
      <div className="max-w-6xl mx-auto">
        {/* AggregateRating Schema for SEO */}
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              '@context': 'https://schema.org',
              '@type': 'Service',
              name: 'DatingAssistent voor 30-plussers',
              aggregateRating: {
                '@type': 'AggregateRating',
                ratingValue: '4.9',
                bestRating: '5',
                worstRating: '1',
                ratingCount: '214',
              },
              review: stories.map(story => ({
                '@type': 'Review',
                reviewRating: {
                  '@type': 'Rating',
                  ratingValue: '5',
                  bestRating: '5',
                },
                author: {
                  '@type': 'Person',
                  name: story.name,
                  age: story.age.toString(),
                },
                reviewBody: story.quote,
                datePublished: '2026-03-01',
              })),
            }),
          }}
        />

        {/* Section header */}
        <motion.div
          className="text-center mb-16"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
        >
          <h2 className="text-3xl md:text-4xl font-bold mb-4" style={{ color: colors.deepPurple }}>
            Zij vonden wat ze zochten
          </h2>
          <p className="text-lg" style={{ color: colors.mediumGray }}>
            Echte verhalen van 30-plussers die bewust kozen voor een andere aanpak
          </p>
        </motion.div>

        {/* Stories grid */}
        <div className="grid md:grid-cols-3 gap-6">
          {stories.map((story, i) => (
            <motion.div
              key={i}
              className="rounded-2xl p-8 bg-white border-2 relative overflow-hidden"
              style={{ borderColor: colors.dustyRose }}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.15, duration: 0.5 }}
              whileHover={{ y: -6, boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}
            >
              {/* Quote icon decoration */}
              <div className="absolute top-4 right-4 opacity-10">
                <Quote className="w-16 h-16" style={{ color: colors.dustyRose }} />
              </div>

              {/* Star rating */}
              <div className="flex gap-1 mb-4">
                {[...Array(5)].map((_, j) => (
                  <Star key={j} className="w-5 h-5 fill-yellow-400 text-yellow-400" />
                ))}
              </div>

              {/* Quote */}
              <blockquote className="mb-6 relative z-10">
                <p className="text-base italic leading-relaxed" style={{ color: colors.charcoal }}>
                  "{story.quote}"
                </p>
              </blockquote>

              {/* Author info */}
              <div className="border-t-2 pt-4" style={{ borderColor: colors.softBlush }}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-bold text-lg" style={{ color: colors.deepPurple }}>
                      {story.name}, {story.age}
                    </p>
                    <p className="text-sm" style={{ color: colors.mediumGray }}>
                      {story.situation}
                    </p>
                  </div>
                </div>

                {/* Program and time badges */}
                <div className="flex flex-wrap gap-2">
                  <Badge
                    className="text-xs font-medium"
                    style={{
                      backgroundColor: `${colors.dustyRose}20`,
                      color: colors.deepPurple,
                      border: 'none',
                    }}
                  >
                    {story.program}
                  </Badge>
                  <Badge
                    className="text-xs font-medium"
                    style={{
                      backgroundColor: `${colors.warmCoral}20`,
                      color: colors.charcoal,
                      border: 'none',
                    }}
                  >
                    ⏱ {story.timeToSuccess}
                  </Badge>
                </div>
              </div>
            </motion.div>
          ))}
        </div>

        {/* Bottom reassurance */}
        <motion.div
          className="mt-12 text-center"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ delay: 0.5 }}
        >
          <p className="text-lg" style={{ color: colors.charcoal }}>
            <span className="font-semibold" style={{ color: colors.deepPurple }}>
              Gemiddelde leeftijd van onze leden: 34 jaar
            </span>
            <br />
            Je bent niet de enige die het anders wil aanpakken
          </p>
        </motion.div>
      </div>
    </section>
  );
}

export default SuccessStories30Plus;
