...
export function WorldClassProfileHub({ embedded = false }: WorldClassProfileHubProps) {
  ...
  function generateSmartInsights(profile: any, status: ProfileStatus): SmartInsight[] {
    ...
    if (!status.hasBio) {
      list.push({ id: 'no-bio', priority: 'high', impact: 40, icon: MessageCircle, color: 'red',
        ...
        actionLabel: 'Bio schrijven', onAction: () => router.push('/dashboard?tab=ai-bio-generator'), source: 'profile' });
      ...
      list.push({ id: 'short-bio', priority: 'high', impact: 25, icon: MessageCircle, color: 'blue',
        ...
        actionLabel: 'Bio uitbreiden', onAction: () => router.push('/dashboard?tab=ai-bio-generator'), source: 'profile' });
      ...
      list.push({ id: 'bio-no-question', priority: 'medium', impact: 15, icon: MessageCircle, color: 'green',
        ...
        actionLabel: 'Bio optimaliseren', onAction: () => router.push('/dashboard?tab=ai-bio-generator'), source: 'profile' });
    }
  ...
  const nextStep = (): { label: string; action: () => void } | null => {
    if (!profileStatus.hasBio)       return { label: 'Schrijf je bio', action: () => router.push('/dashboard?tab=ai-bio-generator') };
  ...
  const quickActions = [
    ...
    { id: 'bio',                   icon: Wand2,        title: 'Bio Coach',              subtitle: 'AI-gedreven bio verbetering',                action: () => router.push('/dashboard?tab=ai-bio-generator') },
