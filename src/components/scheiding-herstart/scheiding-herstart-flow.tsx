...
import type { IntakeData, ScanResult } from './types';

type FlowStep = 'intro' | 'intake' | 'questionnaire' | 'results';
...
export function ScheidingHerstartFlow({ onClose }: ScheidingHerstartFlowProps = {}) {
  ...
  const stepProgress = { intro: 0, intake: 15, questionnaire: 50, results: 100 }[step];
  ...
  const handleQuestionnaireComplete = async (answers: Record<string, number>) => {
    try {
      const response = await fetch('/api/scheiding-herstart', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ answers, intake, userId: user?.id }),
      });
      ...
      // Fallback: calculate client-side if API fails
      const scores = calculateFallbackScores(answers, intake);
      setResult({ scores, aiAnalysis: null });
      setStep('results');
    }
  };
        ...
        <CardContent className="p-6 relative">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-500 text-sm mb-5">
              ...
              {step === 'results' && 'Jouw persoonlijke analyse is klaar'}
            </p>
      ...
      <CardContent className="p-8 space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="text-xs text-gray-400 mt-3">
            Gratis · Geen account vereist · Resultaat altijd opgeslagen als je bent ingelogd
          </p>
        </div>
      </CardContent>
    </Card>
...
function calculateFallbackScores(answers: Record<string, number>, intake: IntakeData) {
