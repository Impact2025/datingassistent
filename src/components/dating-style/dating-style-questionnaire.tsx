...
interface MicroIntake {
  huidigeDatingStatus: string;
  gewensteRelatieType: string;
  appGebruik: string;
}

interface DatingStyleQuestionnaireProps {
  onComplete: (data: any) => void;
  onBack: () => void;
}

export function DatingStyleQuestionnaire({ onComplete, onBack }: DatingStyleQuestionnaireProps) {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await fetch('/api/dating-style/questions');
  ...
  const handleMicroIntakeChange = (field: keyof MicroIntake, value: string) => {
    setMicroIntake(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Start assessment
      const startResponse = await fetch('/api/dating-style', {
        method: 'POST',
      ...
      // Submit responses
      const submitResponse = await fetch('/api/dating-style', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'submit',
              ...
              <CardContent className="space-y-6">
                <div>
                  <Label className="text-base font-semibold mb-3 block">
                    Wat is je huidige dating status?
                  </Label>
                  <Select
                    value={microIntake.huidigeDatingStatus}
                    onValueChange={(value) => handleMicroIntakeChange('huidigeDatingStatus', value)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Selecteer je situatie" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="single">Single, actief aan het daten</SelectItem>
                      <SelectItem value="dating">Aan het daten met iemand</SelectItem>
                      <SelectItem value="relationship">In een relatie</SelectItem>
                      <SelectItem value="break">Net uit een relatie</SelectItem>
                      <SelectItem value="pause">Pauze van dating</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  <Label className="text-base font-semibold mb-3 block">
                    Wat voor relatie zoek je?
                  </Label>
                  <Select
                    value={microIntake.gewensteRelatieType}
                    onValueChange={(value) => handleMicroIntakeChange('gewensteRelatieType', value)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Selecteer je doel" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="casual">Casual dating, geen haast</SelectItem>
                      <SelectItem value="serious">Serieus, op zoek naar partner</SelectItem>
                      <SelectItem value="marriage">Op termijn trouwen</SelectItem>
                      <SelectItem value="friends">Vriendschap eerst</SelectItem>
                      <SelectItem value="open">Nog niet zeker</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  <Label className="text-base font-semibold mb-3 block">
                    Hoe vaak gebruik je dating apps?
                  </Label>
                  <Select
                    value={microIntake.appGebruik}
                    onValueChange={(value) => handleMicroIntakeChange('appGebruik', value)}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Selecteer frequentie" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="frequent">Dagelijks/meerdere keren per week</SelectItem>
                      <SelectItem value="occasional">Een paar keer per maand</SelectItem>
                      <SelectItem value="rare">Zelden/minder dan maandelijks</SelectItem>
                      <SelectItem value="never">Nooit/gebruikt geen apps</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
        ...
        <div className="max-w-2xl mx-auto">
              ...
              {currentQuestion.question_type === 'scenario' && currentQuestion.scenarios ? (
                ...
                >
                  {currentQuestion.scenarios.map((scenario) => (
                    <div key={scenario.id} className="flex items-start space-x-3 p-4 border border-gray-200 rounded-lg hover:border-purple-300 transition-colors">
                      <RadioGroupItem
                ...
                >
                  {[
                    ...
                    <div key={option.value} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:border-purple-300 transition-colors">
                      <RadioGroupItem
          ...
          <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-blue-900 mb-1">Wees eerlijk over je gedrag</h4>
                <p className="text-sm text-blue-800">
                  Deze scan kijkt naar hoe je écht date — niet naar hoe je denkt te daten.
                  Er zijn geen "verkeerde" stijlen, alleen inzichten die je helpen.
                </p>
              </div>
            </div>
          </div>
