...
export async function POST(request: NextRequest) {
  try {
    logger.log('🚀 Starting Monthly Reports System Migration via API...');
    ...
    if (!existingTableNames.includes('monthly_reports')) {
      logger.log('📊 Creating monthly_reports table...');
      await sql`
      ...
      `;
      logger.log('✅ monthly_reports table created');
    } else {
      logger.log('⏭️  monthly_reports table already exists');
    }

    // Check and fix user_goals table structure
    logger.log('🎯 Checking user_goals table structure...');
    ...
    try {
      ...
      if (existingReports.rows[0].count === 0) {
        ...
        if (users.rows.length > 0) {
          ...
          `;
          logger.log('✅ Sample monthly report added');
        }
      } else {
        logger.log('⏭️  Monthly reports already exist');
      }
    } catch (error: any) {
    ...
    logger.log('🧪 Testing monthly reports...');
    ...
    logger.log('🎉 Monthly Reports System Migration Completed Successfully!');
    logger.log('📊 The monthly reports system is now fully operational.');
