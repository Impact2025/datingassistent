...
async function migrate() {
  logger.log('Starting Lead Activation migration...\n');

  try {
    // 1. Add lead_intake column to user_profiles
    logger.log('1. Adding lead_intake column to user_profiles...');
    await sql`
      ALTER TABLE user_profiles
      ADD COLUMN IF NOT EXISTS lead_intake JSONB;
    `;
    logger.log('   ✅ lead_intake column added\n');

    // 2. Add freemium_credits column to users
    logger.log('2. Adding freemium_credits column to users...');
    await sql`
      ALTER TABLE users
      ADD COLUMN IF NOT EXISTS freemium_credits INTEGER DEFAULT 1;
    `;
    logger.log('   ✅ freemium_credits column added (default: 1)\n');

    // 3. Add initial_photo_score column to users
    logger.log('3. Adding initial_photo_score column to users...');
    await sql`
      ALTER TABLE users
      ADD COLUMN IF NOT EXISTS initial_photo_score DECIMAL(3,1);
    `;
    logger.log('   ✅ initial_photo_score column added\n');

    // 4. Add lead_onboarding_completed column to users
    logger.log('4. Adding lead_onboarding_completed column to users...');
    await sql`
      ALTER TABLE users
      ADD COLUMN IF NOT EXISTS lead_onboarding_completed BOOLEAN DEFAULT FALSE;
    `;
    logger.log('   ✅ lead_onboarding_completed column added\n');

    // 5. Add lead_oto_shown column to track if OTO was shown
    logger.log('5. Adding lead_oto_shown column to users...');
    await sql`
      ALTER TABLE users
      ADD COLUMN IF NOT EXISTS lead_oto_shown BOOLEAN DEFAULT FALSE;
    `;
    logger.log('   ✅ lead_oto_shown column added\n');

    // 6. Create index for quick freemium user lookups
    logger.log('6. Creating index for freemium users...');
    await sql`
    ...
    logger.log('========================================');
    logger.log('✅ Lead Activation migration completed!');
    logger.log('========================================\n');

    logger.log('New columns added:');
    logger.log('  - user_profiles.lead_intake (JSONB)');
    logger.log('  - users.freemium_credits (INTEGER, default 1)');
    logger.log('  - users.initial_photo_score (DECIMAL)');
    logger.log('  - users.lead_onboarding_completed (BOOLEAN)');
    logger.log('  - users.lead_oto_shown (BOOLEAN)\n');
