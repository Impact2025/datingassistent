...
const blogs = [
  {
    ...
    publishDate: new Date('2015-02-08'),
    image: '',
    content: `
  ...
  },
  {
    ...
    publishDate: new Date('2015-09-17'),
    image: '',
    content: `
  ...
  },
  {
    ...
    publishDate: new Date('2016-04-23'),
    image: '',
    content: `
  ...
  },
  {
    ...
    publishDate: new Date('2016-11-09'),
    image: '',
    content: `
  ...
  },
  {
    ...
    publishDate: new Date('2017-07-05'),
    image: '',
    content: `
...
async function migrateBlogs() {
  console.log('Starting blog migration to PostgreSQL...\n');
  ...
  try {
    ...
    for (const blog of blogs) {
      try {
        ...
        const existing = await sql`
          SELECT id FROM blog_posts WHERE slug = ${blog.slug}
        `;
        ...
        // Insert blog post
        // PostgreSQL needs proper array syntax, not JSON strings
        const keywordsArray = `{${blog.keywords.map(k => `"${k}"`).join(',')}}`;
        ...
        console.log(`✓ Added: "${blog.title}" (ID: ${result.rows[0].id})`);
        successCount++;
      } catch (error) {
    ...
    console.log(`Success: ${successCount} blog posts`);
    console.log(`Errors: ${errorCount} blog posts`);
    console.log('='.repeat(60));

    // Verify the blogs were added
    console.log('\nVerifying blog posts in database...');
    ...
    console.log(`\n✓ Total blog posts in database: ${allBlogs.rows.length}`);
    console.log('\nRecent blog posts:');
    allBlogs.rows.slice(0, 5).forEach((blog, index) => {
      console.log(`  ${index + 1}. ${blog.title} (views: ${blog.view_count})`);
    });
